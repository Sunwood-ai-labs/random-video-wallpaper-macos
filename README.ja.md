...
- フォルダ選択と開始/停止ができるメニューバーモード
- メイン画面と短いフェードに寄せる低電力モード
...
アプリはメニューバーに常駐します。メニューから動画やフォルダを選び、
ターミナルへ戻らずに停止や表示オプションを切り替えられます。
...
配布用の app bundle、zip、DMG を作成します。

```sh
VERSION=0.2.0 scripts/build-release-app
```

ローカルではデフォルトで ad-hoc 署名します。Gatekeeper の警告を避けて一般配布するには、
`CODESIGN_IDENTITY` に Developer ID Application 証明書を指定し、zip または DMG を
notarize してから公開してください。
